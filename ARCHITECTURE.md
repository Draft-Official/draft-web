...
│   ├── docs/
│   │   └── figma-ui-importer.md      # Figma UI 변환 가이드
│   └── skills/
│
├── public/                           # 정적 파일
...
3. **타입 시스템 적용 및 검증**
   ```
   Phase 2 타입 시스템 적용 (src/shared/types/match.ts)
   Zod 검증 추가 (Form 컴포넌트)
   ```
...
**수행 단계:**
1. Figma Make 코드 분석
2. Draft 구조로 변환 (import script 실행)
3. Phase 2 타입 시스템 적용
4. Registry에 등록
5. Import 경로 업데이트
6. TypeScript 에러 체크

자세한 내용은 [.claude/docs/figma-ui-importer.md](.claude/docs/figma-ui-importer.md)를 참조하세요.
...
Import script가 생성한 파일에는 헤더가 있음:

```typescript
/**
 * 🤖 AUTO-GENERATED by import-figma-component
 * 생성일: {DATE}
 * Script: scripts/import-figma-component.py
 */
```

**수동 수정 주의**: Script 재실행 시 덮어쓰여질 수 있음
...
- [Figma UI Import 가이드](.claude/docs/figma-ui-importer.md)
- [Context 관리 가이드](.claude/docs/context-management.md)
