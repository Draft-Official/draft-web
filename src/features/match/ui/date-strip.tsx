...
export function DateStrip({ selectedDateISO, onSelectDate }: DateStripProps) {
    ...
    return (
        <div className="pt-2 pb-3 px-4">
            <div className="flex gap-2 min-w-max overflow-x-auto no-scrollbar scroll-smooth">
                 <Button
                    variant="outline"
                    onClick={() => onSelectDate(null)}
                    className={cn(
                        "rounded-xl border-slate-200 text-slate-600 bg-white h-[52px] px-3 flex items-center gap-1 shadow-sm transition-all active:scale-95 shrink-0",
                 ...
                 {calendarDates.map((d) => (
                     <button
                        ...
                        className={cn(
                            "flex flex-col items-center justify-center min-w-[48px] h-[52px] rounded-xl border transition-all active:scale-95 shadow-sm outline-none shrink-0",
