...
export function TournamentManageView({
  ...
  const handleRemoveParticipant = (participantId: string) => {
    ...
    toast.success('선수가 명단에서 제거되었습니다.');
  };

  const handleAddParticipant = () => {
    toast.info('선수 추가 기능은 준비중입니다.');
  };
    ...
    <div className={layoutMode === 'split' ? 'min-h-full bg-slate-50 pb-24' : 'min-h-screen bg-slate-50 pb-32'}>
      ...
      <header className="bg-white border-b border-slate-200 px-5 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-slate-900">대회 관리</h1>
        </div>
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 space-y-3">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-bold text-slate-900">
              {tournament.tournamentName} <span className="text-slate-500">·</span>{' '}
              {tournament.round}
            </h2>
          </div>
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="flex items-center justify-between p-5 border-b border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-lg text-slate-900">대회 안내사항</h2>
            </div>
            <button
          ...
          <div className="p-5">
            {isEditingDescription ? (
              ...
              >
                {description || '대회 안내사항을 입력하려면 클릭하세요'}
              </p>
            )}
          </div>
        </section>
