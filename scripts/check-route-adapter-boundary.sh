...
collect_route_adapter_files() {
  if [ -d src/app ]; then
    rg --files src/app | rg '(/page\.tsx$|/(loading|error|not-found)\.tsx$)' || true
  fi

  if [ -d app ]; then
    rg --files app | rg '(/page\.tsx$|/(loading|error|not-found)\.tsx$)' || true
  fi
}

route_adapter_files="$(collect_route_adapter_files | sort -u)"

if [ -z "$route_adapter_files" ]; then
  echo "OK: no route adapter files found."
  exit 0
fi

tmp_file="$(mktemp)"
trap 'rm -f "$tmp_file"' EXIT
printf "%s\n" "$route_adapter_files" > "$tmp_file"
