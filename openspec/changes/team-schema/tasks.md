...
- [ ] 1.1 teams 테이블 확장 마이그레이션 작성 (code, short_intro, description, regular_day, regular_time)
- [ ] 1.2 applications 테이블 확장 마이그레이션 작성 (source, description 컬럼)
- [ ] 1.3 application_status enum에 NOT_ATTENDING 값 추가
- [ ] 1.4 team_fees 테이블 생성 마이그레이션 작성
- [ ] 1.5 team_fees RLS 정책 추가
- [ ] 1.6 기존 데이터 마이그레이션 (teams.code 기본값, applications.source 기본값)
- [ ] 1.7 Supabase에 마이그레이션 적용 및 검증

## 2. Team Feature API Layer

- [ ] 2.1 src/features/team/ 디렉토리 구조 생성 (api/, model/, ui/)
- [ ] 2.2 team/model/types.ts 타입 정의 (Team, TeamMember, TeamRole, TeamMemberStatus)
- [ ] 2.3 team/api/team-api.ts 작성 (getTeam, getTeamByCode, createTeam, updateTeam)
- [ ] 2.4 team/api/team-mapper.ts 작성 (DB row → Client type 변환)
- [ ] 2.5 team/api/keys.ts 작성 (React Query 키 정의)
- [ ] 2.6 team/api/queries.ts 작성 (useTeam, useTeamByCode, useMyTeams)
- [ ] 2.7 team/api/mutations.ts 작성 (useCreateTeam, useUpdateTeam)

## 3. Team Membership API Layer

- [ ] 3.1 team/api/membership-api.ts 작성 (getTeamMembers, createJoinRequest, updateMemberStatus, removeMember)
- [ ] 3.2 team/api/membership-queries.ts 작성 (useTeamMembers, useMyMembership)
- [ ] 3.3 team/api/membership-mutations.ts 작성 (useJoinTeam, useApproveJoin, useRejectJoin, useRemoveMember, useUpdateRole)

## 4. Team Match & Voting API Layer

- [ ] 4.1 application/model/types.ts에 source 타입 추가 (ApplicationSource)
- [ ] 4.2 application/api에 팀 투표 관련 함수 추가 (createTeamVote, updateTeamVote)
- [ ] 4.3 team/api/team-match-api.ts 작성 (createTeamMatch, getTeamMatches, closeVoting)
- [ ] 4.4 team/api/team-match-queries.ts 작성 (useTeamMatches, useVotingStatus)
- [ ] 4.5 team/api/team-match-mutations.ts 작성 (useCreateTeamMatch, useVote, useCloseVoting)

## 5. Team Fees API Layer

- [ ] 5.1 team/api/fees-api.ts 작성 (getTeamFees, updateFeeStatus)
- [ ] 5.2 team/api/fees-queries.ts 작성 (useTeamFees, useMyFeeStatus)
- [ ] 5.3 team/api/fees-mutations.ts 작성 (useUpdateFeeStatus)
...
- [ ] 12.1 shared/config/team-constants.ts 생성 (TEAM_ROLE_VALUES, TEAM_MEMBER_STATUS_VALUES 등)
- [ ] 12.2 shared/types/team.types.ts 생성 (필요시 공유 타입)
- [ ] 12.3 application source 상수 추가 (APPLICATION_SOURCE_VALUES)
